...
const endRouteButton = document.getElementById('end-route-button');

endRouteButton.addEventListener('click', function() {
    endRoute();
});
...
function endRoute() {
  ...
  document.getElementById('end-route-button').style.display = 'none';
...
function setMapHeight() {
  ...
  const formHeightVh = (formHeight / window.innerHeight) * 100;
  document.getElementById('map').style.height = `calc(100vh - ${formHeightVh}vh)`;
}
...
document.getElementById('address-form').addEventListener('submit', function(e) {
  ...
  const address = (document.getElementById('address-input').value.toLowerCase() === 'home') ? '5160 Linnean Terrace NW' : document.getElementById('address-input').value;
      ...
      if (navigator.geolocation) {
        directionsService.route({
          ...
          if (status === 'OK') {
            ...
            destinationMarker = makeMarker(route.legs[0].end_location, "https://img.icons8.com/color/48/map-pin.png");
              ...
              if (route && pos) {
                directionsService.route({
                  ...
                  if (status === 'OK') {
                    ...
                    let etaText = routeETA.legs[0].duration.text;
                    if (etaText.includes('hours') && etaText.includes('mins')) {
                      let etaInSeconds = routeETA.legs[0].duration.value;
                      let etaInHours = etaInSeconds / 3600;
                      document.getElementById('eta').innerHTML = etaInHours.toFixed(1) + ' hours';
                    } else {
                      document.getElementById('eta').innerHTML = etaText;
                    }
                    let currentTime = new Date().getTime() / 1000;
                    let arrivalTimeInSeconds = currentTime + route.legs[0].duration.value;
                    let arrivalTime = new Date(arrivalTimeInSeconds * 1000);
                    let arrivalTimeString = arrivalTime.toLocaleTimeString('en-US', { hour: 'numeric', hour12: true, minute: 'numeric' });
                    document.getElementById('arrive').innerHTML = arrivalTimeString;
                  }
                });
              }
            }
            ...
            document.getElementById('end-route-button').style.display = 'block';
          } else {
